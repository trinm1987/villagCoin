...
*Setup instructions for a Gitian build of VillageCoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the VillageCoin
...
- [Building VillageCoin Core](#building-village-core)
...
Clone the git repositories for VillageCoin Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/dashpay/village
...
in the VillageCoin Core repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building VillageCoin Core
----------------

To build VillageCoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the VillageCoin Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/village/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/dashpay/village
...
For example:
```bash
URL=https://github.com/crowning-/village.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit village=${COMMIT} --url village=${URL} ../village/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit village=${COMMIT} --url village=${URL} ../village/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit village=${COMMIT} --url village=${URL} ../village/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the village git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../village/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/village
SIGPATH=/some/root/path/village-detached-sigs

./bin/gbuild --url village=${BTCPATH},signature=${SIGPATH} ../village/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/village-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/village-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/village-osx-build.assert
```
...
[village/gitian.sigs](https://github.com/dashpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
